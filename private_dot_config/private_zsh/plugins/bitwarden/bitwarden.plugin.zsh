...
function .bw_ensure_unlocked() {
	local bw_status=$(bw status --session "$BW_SESSION" 2>/dev/null | jq -r '.status')
...
function .bw_select() {
	jq -r ".[] | [.name, $1] | join(\"$ZSH_BITWARDEN_DELIMITER\")" <(bw list items --session "$BW_SESSION" --nointeraction 2>&/dev/null) | fzf -0 --with-nth 1 -d "$ZSH_BITWARDEN_DELIMITER"
}
